...
after it from registering.

The failure is never discarded: it is recorded in :data:`FAILED_MODULES`,
warned about by name, and reported again by
...
def _discover() -> None:
    ...
    for module_info in pkgutil.iter_modules([str(package_dir)]):
        ...
        try:
            ...
            # A prior module is effectively a plugin, so the exceptions it can
            # raise at import time are not enumerable in advance; this clause
            # must not be narrowed to ImportError. The failure is recorded and
